...
views in the portable version-6 workspace format, which also retains the four
KUtrace lane groups in full, highlighted-only, or hidden states, the vertical
row scale and position, and original multi-state display modes; version-1
through version-5 files remain importable. The original self-contained viewer is a separate compatibility tab
