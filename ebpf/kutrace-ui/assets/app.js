...
async function drawTimeline(signal){
  ...
  canvas.dataset.ready='false';canvas.dataset.trackMode=track;canvas.dataset.highlightedTracks=[...state.highlightedTracks].join(',');state.trackRegions=[];updateRangeChrome();
  if(track==='cpu_pid'){
    ...
    const raw=await query(rawTimelineSql(start,end,trackScope,'cpu'),DETAIL_EVENT_LIMIT,signal),selected=new Set(tracks),rows=[];for(const row of raw.rows){const cpuKey=`cpu:${row[5]}`,pidKey=`pid:${row[6]}`,rpcKey=`rpc:${row[13]}`,resourceKey=`res:${row[10]}`;if(selected.has(cpuKey)){const copy=row.slice();copy[4]=cpuKey;rows.push(copy)}if(selected.has(pidKey)){const copy=row.slice();copy[4]=pidKey;rows.push(copy)}if(selected.has(rpcKey)){const copy=row.slice();copy[4]=rpcKey;rows.push(copy)}if(['resource','lock'].includes(row[9])&&selected.has(resourceKey)){const copy=row.slice();copy[4]=resourceKey;rows.push(copy)}}
    ...
    canvas.dataset.visibleTracks=tracks.join(',');installTimelineNavigation(canvas,width);renderSelectionOverlay();canvas.dataset.preview='false';canvas.dataset.ready='true';return;
  }
  ...
  canvas.dataset.visibleTracks=tracks.map(value=>`${track}:${value}`).join(',');installTimelineNavigation(canvas,width);renderSelectionOverlay();canvas.dataset.preview='false';canvas.dataset.ready='true';
}
