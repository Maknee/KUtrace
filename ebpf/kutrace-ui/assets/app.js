...
function drawKutraceTimeline(canvas,width,ratio,start,end,tracks,result){
  ...
  for(const [index,value] of tracks.entries()){const row=layout.get(value);ctx.fillStyle=index%2?'#fbfbfb':'#fff';ctx.fillRect(0,row.y,width,row.rowH);ctx.fillStyle='#f6f7f8';ctx.fillRect(0,row.y,TIMELINE_LABEL_WIDTH,row.rowH);ctx.strokeStyle='#d5d9df';ctx.beginPath();ctx.moveTo(0,row.y+row.rowH-.5);ctx.lineTo(width,row.y+row.rowH-.5);ctx.stroke();ctx.fillStyle='#0836c7';ctx.font='bold 12px ui-monospace,monospace';ctx.fillText(`${state.trackMode.toUpperCase()} ${value}`,8,row.y+row.rowH/2+4)}drawKutraceGrid(ctx,width,height,start,end);
