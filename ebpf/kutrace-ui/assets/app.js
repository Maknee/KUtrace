const state={
  ...
  selectedAgentSql:'',refreshController:null,overviewController:null,overviewKey:'',timelineFrame:null,
  overviewRows:[],hitRegions:[],trackRegions:[],highlightedTracks:new Set(),search:'',searchInvert:false,followTail:false,
  ...
  groups:{cpu:true,process:true,rpc:true,resource:true,kernel:true,agent:true}
};
...
function visibilitySql(){
  ...
  if((state.trackMode==='cpu'&&!state.groups.cpu)||(state.trackMode==='pid'&&!state.groups.process)||(state.trackMode==='cpu_pid'&&!state.groups.cpu&&!state.groups.process&&!state.groups.rpc&&!state.groups.resource))hidden.push('0');
...
function snapshotTimeline(){const canvas=$('timeline'),source=document.createElement('canvas');source.width=canvas.width;source.height=canvas.height;if(source.width&&source.height)source.getContext('2d').drawImage(canvas,0,0);return source}
function previewTimelineRange(previous,next,source=null){
  ...
  source=source||snapshotTimeline();const ratio=canvas.width/width,label=TIMELINE_LABEL_WIDTH*ratio,plot=canvas.width-label,ctx=canvas.getContext('2d');ctx.save();ctx.setTransform(1,0,0,1,0,0);ctx.fillStyle=state.timelineRenderer==='kutrace'?'#fff':'#080b11';ctx.fillRect(label,0,plot,canvas.height);
  ...
  ctx.restore();canvas.dataset.preview='true';canvas.dataset.previewMode='bitmap';canvas.dataset.ready='false';
}
function redrawCachedTimeline(next){
  const frame=state.timelineFrame;if(!frame||frame.renderer!==state.timelineRenderer||frame.trackMode!==state.trackMode)return false;const canvas=$('timeline'),width=canvas.getBoundingClientRect().width||800,ratio=devicePixelRatio||1,complete=next[0]>=frame.coverage[0]&&next[1]<=frame.coverage[1];
  if(frame.renderer==='kutrace')drawKutraceTimeline(canvas,width,ratio,next[0],next[1],frame.tracks,frame.result);else drawDetailedTimeline(canvas,width,ratio,next[0],next[1],frame.tracks,frame.result);canvas.dataset.preview='true';canvas.dataset.previewMode=complete?'vector':'vector-partial';canvas.dataset.ready='false';canvas.dataset.highlightedTracks=[...state.highlightedTracks].join(',');installTimelineNavigation(canvas,width);renderSelectionOverlay();return true;
}
function setRange(start,end){const previous=state.range?[...state.range]:null,next=boundedRange(start,end);state.range=next;if(previous&&!redrawCachedTimeline(next))previewTimelineRange(previous,next);updateOverviewViewport();updateRangeChrome();return refresh()}
...
function trackLabel(value){
  const text=String(value);if(text.startsWith('cpu:'))return `CPU ${text.slice(4)}`;if(text.startsWith('pid:'))return `PID ${text.slice(4)}`;if(text.startsWith('rpc:'))return `RPC ${text.slice(4)}`;if(text.startsWith('res:'))return `RES ${text.slice(4)}`;return `${state.trackMode.toUpperCase()} ${text}`;
}
function trackFieldValue(value){const match=/^(cpu|pid|rpc|res):(-?\d+)$/.exec(String(value));return match?{field:match[1]==='res'?'arg0':match[1],value:Number(match[2])}:{field:state.trackMode,value:Number(value)}}
function trackEmphasized(value){return !state.highlightedTracks.size||state.highlightedTracks.has(String(value))}
function toggleTrackHighlight(value){const key=String(value);if(state.highlightedTracks.has(key))state.highlightedTracks.delete(key);else state.highlightedTracks.add(key);$('timeline').dataset.highlightedTracks=[...state.highlightedTracks].join(',');refresh()}
...
function installTimelineNavigation(canvas,width){
  ...
  const trackAt=event=>{const hit=hitAt(event.offsetX,event.offsetY);if(hit)return hit.track;const row=state.trackRegions.find(region=>event.offsetY>=region.y&&event.offsetY<region.y+region.h);return row&&row.track};
  canvas.onpointerdown=event=>{
    if(event.button!==0)return;if(event.shiftKey){const track=trackAt(event);if(track!==undefined){event.preventDefault();hideTimelineTooltip();toggleTrackHighlight(track)}return}if(event.offsetX<plotStart)return;hideTimelineTooltip();drag={x:plotX(event),last:plotX(event),range:[...state.range],pan:event.altKey,moved:false,pointer:event.pointerId};canvas.setPointerCapture(event.pointerId);event.preventDefault();
...
function exactTimelineSql(start,end,bucket,bucketCount,trackScope,track='cpu'){return `WITH RECURSIVE scoped_events(${track},event,name,category,ipc,ts,ts_end,bucket,last_bucket) AS (
  ...
  ranked AS (SELECT *,ROW_NUMBER() OVER(PARTITION BY bucket,${track},category ORDER BY weight DESC,count DESC,event,name,ipc) rank FROM scored)
  SELECT bucket,${track},event,name,category,ipc,weight,count FROM ranked WHERE rank=1 ORDER BY ${track},bucket,category`}
function mipmapTimelineSql(start,end,bucket,bucketCount,cpuScope,table){
  ...
  ranked AS (SELECT *,ROW_NUMBER() OVER(PARTITION BY bucket,cpu,category ORDER BY weight DESC,count DESC,event,name,ipc) rank FROM scored)
  SELECT bucket,cpu,event,name,category,ipc,weight,count FROM ranked WHERE rank=1 ORDER BY cpu,bucket,category`;
}
...
function timelineCacheRange(start,end){const margin=(end-start)*.6;return boundedRange(start-margin,end+margin)}
...
function drawKutraceSpan(ctx,x,w,center,bandH,event,name,category,arg0,ipc,matched,selected,emphasized=true){
  const kind=kutraceEventKind(event,category),[light,dark]=kutraceColors(event);ctx.save();ctx.globalAlpha=emphasized?(state.search&&!matched?.16:.96):.14;
...
function drawKutraceTimeline(canvas,width,ratio,start,end,tracks,result){
  const viewportHeight=canvas.closest('.timeline-scroll').clientHeight,{height,layout}=makeTrackLayout(tracks,viewportHeight,78,36),plotWidth=Math.max(1,width-TIMELINE_LABEL_WIDTH),ctx=prepareTimelineCanvas(canvas,width,height,ratio);ctx.fillStyle='#fff';ctx.fillRect(0,0,width,height);state.hitRegions=[];state.trackRegions=tracks.map(track=>{const row=layout.get(track);return {track,y:row.y,h:row.rowH}});
...
function drawSummaryTimeline(canvas,width,ratio,start,end,tracks,result,bucket,pixelPerBucket,source,level){
  ...
  drawTimelineGrid(ctx,width,height,start,end);
  for(const [bin,value,event,name,category,ipc] of result.rows){const x=TIMELINE_LABEL_WIDTH+bin*pixelPerBucket,row=layout.get(value);if(!row)continue;const laneCount=4,laneH=Math.max(2,(row.rowH-4)/laneCount),lane=timelineLane(category),h=Math.max(1,laneH-1),y=row.y+2+lane*laneH,w=Math.ceil(pixelPerBucket),hit={x,y,w,h,track:value,event,name,category,start:start+bin*bucket,end:Math.min(end,start+(bin+1)*bucket)};ctx.fillStyle=palette[category]||palette.special;ctx.globalAlpha=(state.search&&!searchMatch(hit)?.2:1)*(trackEmphasized(value)?1:.16);ctx.fillRect(x,y,w,h);ctx.globalAlpha=1;if(state.search&&searchMatch(hit)){ctx.strokeStyle='#fff';ctx.strokeRect(x+.5,y+.5,Math.max(1,w-1),h-1)}if(ipc&&state.overlays.ipc){ctx.fillStyle='#fff';ctx.fillRect(x,y,Math.max(1,w),2)}state.hitRegions.push(hit)}
...
async function drawTimeline(signal){
  ...
  canvas.dataset.ready='false';canvas.dataset.trackMode=track;canvas.dataset.highlightedTracks=[...state.highlightedTracks].join(',');state.trackRegions=[];state.timelineFrame=null;updateRangeChrome();
  if(track==='cpu_pid'){
    canvas.dataset.trackGroups='cpu,pid,rpc,res';
    const visiblePidSpans=where(`ts < ${end} AND ts_end > ${start} AND dur>0 AND pid>0 AND cpu>=0`),visibleRpcSpans=where(`ts < ${end} AND ts_end > ${start} AND dur>0 AND rpc>0`),visibleResourceSpans=where(`ts < ${end} AND ts_end > ${start} AND dur>0 AND arg0>0 AND category IN ('resource','lock')`),[cpuResult,pidResult,rpcResult,resourceResult]=await Promise.all([
      state.groups.cpu&&state.groups.process?query(`SELECT DISTINCT cpu FROM events ${visiblePidSpans} ORDER BY cpu LIMIT 10000`,10000,signal):Promise.resolve({rows:[]}),
      state.groups.process?query(`SELECT DISTINCT pid FROM events ${visiblePidSpans} ORDER BY pid LIMIT 10000`,10000,signal):Promise.resolve({rows:[]}),
      state.groups.rpc?query(`SELECT DISTINCT rpc FROM events ${visibleRpcSpans} ORDER BY rpc LIMIT 10000`,10000,signal):Promise.resolve({rows:[]}),
      state.groups.resource?query(`SELECT DISTINCT arg0 FROM events ${visibleResourceSpans} ORDER BY arg0 LIMIT 10000`,10000,signal):Promise.resolve({rows:[]})
    ]),allTracks=[...cpuResult.rows.map(row=>`cpu:${row[0]}`).filter(value=>state.timelineRenderer!=='kutrace'||Number(value.slice(4))>=0),...pidResult.rows.map(row=>`pid:${row[0]}`),...rpcResult.rows.map(row=>`rpc:${row[0]}`),...resourceResult.rows.map(row=>`res:${row[0]}`)],pageCount=Math.max(1,Math.ceil(allTracks.length/state.cpuPageSize));
    state.cpuPage=Math.min(state.cpuPage,pageCount-1);const first=state.cpuPage*state.cpuPageSize,tracks=allTracks.slice(first,first+state.cpuPageSize),cpuValues=tracks.filter(value=>value.startsWith('cpu:')).map(value=>Number(value.slice(4))),pidValues=tracks.filter(value=>value.startsWith('pid:')).map(value=>Number(value.slice(4))),rpcValues=tracks.filter(value=>value.startsWith('rpc:')).map(value=>Number(value.slice(4))),resourceValues=tracks.filter(value=>value.startsWith('res:')).map(value=>Number(value.slice(4))),scopeParts=[];if(cpuValues.length)scopeParts.push(`cpu IN (${cpuValues.join(',')})`);if(pidValues.length)scopeParts.push(`pid IN (${pidValues.join(',')})`);if(rpcValues.length)scopeParts.push(`rpc IN (${rpcValues.join(',')})`);if(resourceValues.length)scopeParts.push(`(arg0 IN (${resourceValues.join(',')}) AND category IN ('resource','lock'))`);const trackScope=scopeParts.length?`(${scopeParts.join(' OR ')})`:'0';
    $('cpu-controls').hidden=pageCount===1;$('previous-cpus').disabled=state.cpuPage===0;$('next-cpus').disabled=state.cpuPage>=pageCount-1;$('cpu-window').textContent=allTracks.length?`Rows ${first+1}–${first+tracks.length} of ${allTracks.length}`:'No visible KUtrace groups';
    let coverage=timelineCacheRange(start,end),raw=await query(rawTimelineSql(coverage[0],coverage[1],trackScope,'cpu'),DETAIL_EVENT_LIMIT,signal);if(raw.truncated&&(coverage[0]!==start||coverage[1]!==end)){coverage=[start,end];raw=await query(rawTimelineSql(start,end,trackScope,'cpu'),DETAIL_EVENT_LIMIT,signal)}const selected=new Set(tracks),rows=[];for(const row of raw.rows){const cpuKey=`cpu:${row[5]}`,pidKey=`pid:${row[6]}`,rpcKey=`rpc:${row[13]}`,resourceKey=`res:${row[10]}`;if(selected.has(cpuKey)){const copy=row.slice();copy[4]=cpuKey;rows.push(copy)}if(selected.has(pidKey)){const copy=row.slice();copy[4]=pidKey;rows.push(copy)}if(selected.has(rpcKey)){const copy=row.slice();copy[4]=rpcKey;rows.push(copy)}if(['resource','lock'].includes(row[9])&&selected.has(resourceKey)){const copy=row.slice();copy[4]=resourceKey;rows.push(copy)}}
    let height;if(!raw.truncated){const result={...raw,rows};height=state.timelineRenderer==='kutrace'?drawKutraceTimeline(canvas,width,ratio,start,end,tracks,result):drawDetailedTimeline(canvas,width,ratio,start,end,tracks,result);state.timelineFrame={renderer:state.timelineRenderer,trackMode:track,coverage,tracks:[...tracks],result}}
    if(height===undefined){const bucketCount=Math.max(1,Math.min(Math.floor(plotWidth),Math.floor(8000/Math.max(1,tracks.length)))),bucket=(end-start)/bucketCount,pixelPerBucket=plotWidth/bucketCount,requests=[];if(cpuValues.length)requests.push(query(exactTimelineSql(start,end,bucket,bucketCount,`cpu IN (${cpuValues.join(',')})`,'cpu'),10000,signal).then(result=>result.rows.map(row=>[row[0],`cpu:${row[1]}`,...row.slice(2)])));if(pidValues.length)requests.push(query(exactTimelineSql(start,end,bucket,bucketCount,`pid IN (${pidValues.join(',')})`,'pid'),10000,signal).then(result=>result.rows.map(row=>[row[0],`pid:${row[1]}`,...row.slice(2)])));if(rpcValues.length)requests.push(query(exactTimelineSql(start,end,bucket,bucketCount,`rpc IN (${rpcValues.join(',')})`,'rpc'),10000,signal).then(result=>result.rows.map(row=>[row[0],`rpc:${row[1]}`,...row.slice(2)])));if(resourceValues.length)requests.push(query(exactTimelineSql(start,end,bucket,bucketCount,`arg0 IN (${resourceValues.join(',')}) AND category IN ('resource','lock')`,'arg0'),10000,signal).then(result=>result.rows.map(row=>[row[0],`res:${row[1]}`,...row.slice(2)])));const summaryRows=(await Promise.all(requests)).flat();height=state.timelineRenderer==='kutrace'?drawKutraceSummary(canvas,width,ratio,start,end,tracks,{rows:summaryRows},bucket,pixelPerBucket,'events','none'):drawSummaryTimeline(canvas,width,ratio,start,end,tracks,{rows:summaryRows},bucket,pixelPerBucket,'events','none')}
    canvas.dataset.visibleTracks=tracks.join(',');installTimelineNavigation(canvas,width);renderSelectionOverlay();canvas.dataset.preview='false';canvas.dataset.previewMode='none';canvas.dataset.ready='true';return;
  }
  ...
  let height;if(!useMipmap){let coverage=timelineCacheRange(start,end),raw=await query(rawTimelineSql(coverage[0],coverage[1],trackScope,track),DETAIL_EVENT_LIMIT,signal);if(raw.truncated&&(coverage[0]!==start||coverage[1]!==end)){coverage=[start,end];raw=await query(rawTimelineSql(start,end,trackScope,track),DETAIL_EVENT_LIMIT,signal)}if(!raw.truncated){height=state.timelineRenderer==='kutrace'?drawKutraceTimeline(canvas,width,ratio,start,end,tracks,raw):drawDetailedTimeline(canvas,width,ratio,start,end,tracks,raw);state.timelineFrame={renderer:state.timelineRenderer,trackMode:track,coverage,tracks:[...tracks],result:raw}}}
  if(height===undefined){const result=await query(sql,50000,signal);height=state.timelineRenderer==='kutrace'?drawKutraceSummary(canvas,width,ratio,start,end,tracks,result,bucket,pixelPerBucket,useMipmap?'mipmap':'events',useMipmap?(useCoarse?'coarse':'fine'):'none'):drawSummaryTimeline(canvas,width,ratio,start,end,tracks,result,bucket,pixelPerBucket,useMipmap?'mipmap':'events',useMipmap?(useCoarse?'coarse':'fine'):'none')}
  canvas.dataset.visibleTracks=tracks.map(value=>`${track}:${value}`).join(',');installTimelineNavigation(canvas,width);renderSelectionOverlay();canvas.dataset.preview='false';canvas.dataset.previewMode='none';canvas.dataset.ready='true';
}
...
const smoothNavigation={keys:new Set(),frame:0,last:0,baseRange:null,source:null};
function navigationInputBlocked(event){const target=event.target;return (target&&typeof target.matches==='function'&&target.matches('input,textarea,select,[contenteditable=true]'))||event.ctrlKey||event.metaKey||event.altKey}
function applySmoothNavigation(seconds){
  if(!state.range||!smoothNavigation.baseRange)return;const left=smoothNavigation.keys.has('a'),right=smoothNavigation.keys.has('d'),zoomIn=smoothNavigation.keys.has('w'),zoomOut=smoothNavigation.keys.has('s');if(!left&&!right&&!zoomIn&&!zoomOut)return;
  let [start,end]=state.range,span=end-start,center=(start+end)/2;if(zoomIn!==zoomOut){const factor=Math.exp((zoomOut?1:-1)*1.8*seconds);span*=factor;start=center-span/2;end=center+span/2}if(left!==right){const delta=(right?1:-1)*span*.72*seconds;start+=delta;end+=delta}const next=boundedRange(start,end);state.range=next;if(!redrawCachedTimeline(next))previewTimelineRange(smoothNavigation.baseRange,next,smoothNavigation.source);updateOverviewViewport();updateRangeChrome();
}
function smoothNavigationFrame(now){
  if(!smoothNavigation.keys.size){smoothNavigation.frame=0;return}const seconds=Math.min(.05,Math.max(0,(now-smoothNavigation.last)/1000));smoothNavigation.last=now;if(seconds)applySmoothNavigation(seconds);smoothNavigation.frame=requestAnimationFrame(smoothNavigationFrame);
}
function beginSmoothNavigation(key){
  if(!smoothNavigation.keys.size){if(state.refreshController)state.refreshController.abort();smoothNavigation.baseRange=[...state.range];smoothNavigation.source=snapshotTimeline();smoothNavigation.last=globalThis.performance.now()}smoothNavigation.keys.add(key);applySmoothNavigation(1/30);if(!smoothNavigation.frame)smoothNavigation.frame=requestAnimationFrame(smoothNavigationFrame);
}
function endSmoothNavigation(key){
...
$('track-mode').onchange=event=>{state.trackMode=event.target.value;state.cpuPage=0;state.highlightedTracks.clear();state.overviewKey='';refresh()};
...
window.addEventListener('keyup',handleNavigationKeyUp);window.addEventListener('blur',()=>{if(!smoothNavigation.keys.size)return;smoothNavigation.keys.clear();if(smoothNavigation.frame)cancelAnimationFrame(smoothNavigation.frame);smoothNavigation.frame=0;smoothNavigation.baseRange=null;smoothNavigation.source=null;refresh()});
