...
//
// compile with g++ -O2 -pthread cc4.cc dclab_log.cc dclab_rpc.cc kutrace_lib.h -o cc4
//
...
rm -rf server4_2025*
rm -rf cc4_2025*
./cc4 localhost 12345 -rep 10 write -key "kkkkk" -value "vvvvv" 1000
./cc4 localhost 12345 quit
./dumplogfile4 "Write 1MB" cc4_2025.log > client4.json
./makeself show_rpc_num.html client4.json client4.html
cp client4.html /ssd/temp/

./dumplogfile4 "Write 1MB" cc4_20250* > client4.json

./dumplogfile4 -all "Write 1MB" cc4_2025* server4_2025* > client4.json
./makeself show_rpc_num.html client4.json client4.html
cp client4.html /ssd/temp/
...
kutrace::RPCLogger logger("cc4_2025.log");
...
bool SendCommand(int sockfd, uint32* randseed, const char* command, 
  ...
  logger.log_client_send(request.header->rpcid, 0, kutrace::NetworkAddress(rpcheader.client_ip, rpcheader.client_port), kutrace::NetworkAddress(rpcheader.server_port, rpcheader.server_port), request.header->method, kutrace::RPCMessage(request.data, request.datalen));
  ...
  logger.log_client_recv(response.header->rpcid, 0, kutrace::NetworkAddress(rpcheader.client_ip, rpcheader.client_port), kutrace::NetworkAddress(rpcheader.server_port, rpcheader.server_port), response.header->method, kutrace::RPCStatus::Success, kutrace::RPCMessage(response.data, response.datalen), response.header->req_send_timestamp, response.header->req_rcv_timestamp, response.header->resp_send_timestamp);
